...
const BarrelOnPlatform = resolveAssetSource(
  require('./barrel-on-platform.gif')
);
const BarrelOnLadder = resolveAssetSource(require('./barrel-on-ladder.gif'));
const BarrelOnPlatformSpecial = resolveAssetSource(
  require('./barrel-on-platform-special.gif')
);
const BarrelOnLadderSpecial = resolveAssetSource(
  require('./barrel-on-ladder-special.gif')
);
...
export const Regular = (world, pos) => {
  ...
  let body = Matter.Bodies.circle(pos.x, pos.y, 12.5, {
    ...
    force: { x: 4.5, y: 0 },
    collisionFilter: {
      category: collisionsCategories.barrel,
      mask:
        collisionsCategories.mario |
        collisionsCategories.barrier |
        collisionsCategories.platform |
        collisionsCategories.barrel
    }
  });
