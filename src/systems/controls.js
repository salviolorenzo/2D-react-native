import { find, filter, any, truthy, base, top, distance } from '../utils';
import { nearBase, aboveTop, closest } from '../utils/ladders';
import { closestAbove. closestBelow, aboveTopEdge, standing} from '../utils/platforms';
...
export default ( entities, {events}) => {
  ...
  let modes = [
   ...
   { 
    ...
    {
      if: 
        grounded && nearBaseOfLadder && mario.controls.mode === 'ladder' && any([swipeLeft, swipeRight, tap], truthy),
      then: () => {
        mario.controls.mode = 'platform';
        mario.direction.horizontal = current.ladder.exitDirection;
        mario.controls.gestures = {}
