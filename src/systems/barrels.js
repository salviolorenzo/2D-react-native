...
const rollDownLadders = entities => {
  let ladders = filter(entities, e => e.loader && e.ladder.acceptBarrels);
  let platforms = filter(entities, 'platform');
  let barrels = filter(entities, 'barrel').filter(b => {
...
const knockOverMario = (entities, dispatch) => {
  ...
  barrels.forEach(b => {
    if (
      ...
      ((b.action == 'on-platform' && collide(mario, left(b))) ||
        collide(mario, right(b)))
    ) {
      mario.action = 'dead';
