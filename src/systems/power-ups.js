...
const MarioWalking = resolveAssetSource(
  require('../components/mario/mario-walking@1x.gif');
);

const MarioIdlingHammering = resolveAssetSource(
  require('../components/mario/mario-idling-hammering');
);

const MarioWalkingHammering = resolveAssetSource(
  require('../components/mario/mario-walking-hammering@1x.gif');
);
...
const destroyBarrels = (entities, dispatch) => {
  ...
  barrelKeys.forEach(key => {
    ...
    let inQuadrant = dir == 'left'  ? barrelPos.x <= marioBase.x : barrelPos.x >= marioBase.x;

    if (
      distance(barrelPos, marioBase) < 50 &&
      barrelPos.y < marioBase.y && inQuadrant
    ) {
