...
const removeEntitiesThatHaveFallenTooFar = entities => {
  ...
  let removal = Object.keys(entities).filter(
    key =>
      key != 'mario' &&
      entities[key].body &&
  ...
  removeAllListeners.forEach(key => {
    remove(key, entities);
  });
...
const updatePlatformCollisionFilters = entities => {
  ...
  let active = filter(platforms, p =>
    aboveTopEdge(p, shift(base(mario), 0, -2))
  );
  let others = filter(platforms, x => active.indexOf(x) === -1);

  active.forEach(x => {
    x.body.collisionFilter.mask =
      collisionsCategories.mario | collisionsCategories.barrel;
  });

  others.forEach(x => {
    x.body.collisionFilter.mask = collisionsCategories.barrel;
  });
