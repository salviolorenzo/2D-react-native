...
const animate = (animations, time) => {
  animations
    .filter(a => a.animation.start <= time.current)
    .forEach(a => {
      ...
      if (percent <= 1) {
        ...
        if (a.animation.complete) {
          a.animation.complete(a.entity);
        }
...
export default (entities, { time, event }) => {
